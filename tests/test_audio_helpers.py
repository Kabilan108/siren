...
from siren.audio import ensure_16k_wav, is_16khz_wav
from siren.models import is_parakeet_model
...
async def test_ensure_16k_wav_skips_conversion(tmp_path):
    audio_path = tmp_path / "audio.wav"
    write_wav(audio_path, sample_rate=16000, channels=1)
    with patch("siren.audio.convert_to_16k_wav", AsyncMock()) as convert:
...
async def test_ensure_16k_wav_converts(tmp_path):
    ...
    write_wav(audio_path, sample_rate=8000, channels=1)
    with patch(
        "siren.audio.convert_to_16k_wav", AsyncMock(return_value=str(converted_path))
    ) as convert:
        result = await ensure_16k_wav(str(audio_path))
